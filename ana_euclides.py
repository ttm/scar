...
for compositor in compositores:
    ...
    for hum in hums:
                    ...
                    for j in xrange(len(inter)):
                        ...
                        if len(trecho) < len(inter_scar): # preciso de notas
                            ...
                            if len(completado) == len(inter_scar):
                                ...
                                dist=[i,j,i,j+len(inter_scar)-1,trecho, sum( [(completado[x]-inter_scar[x])**2 for x in xrange(len(inter_scar))] )**0.5]
                                d = [i,j,i+k+1,falta-1,completado,dist]
                            ...
                            dist=[i,j,i,j+len(inter_scar)-1,trecho, sum( [(trecho[x]-inter_scar[x])**2 for x in xrange(len(inter_scar))] )**0.5]
                            d = [i,j,i,j+len(inter_scar)-1,trecho,dist]
                        if c != None:
                            inci = [c[4][x]-tol <= inter_scar[x] <= c[4][x]+tol for x in xrange(len(inter_scar))]
                            if sum(inci) == len(inter_scar):
                                C_tol.append(c)
                        if tol == 0: # para criar distancias somente na primeira pasasda
                            Distancias.append(dist)
                C_inter.append(C_tol)
                #D_inter.append(D_tol)
            C.append(C_inter)
            #D.append(D_inter)
        ...
        for i in xrange(len(D)):
            inci=D[i]
            print '-' * 80
            print '%s\t%s\t%s\t%s\t%s\tdistancia:%f' % (inci[0]+1, inci[1]+1, inci[2]+1, inci[3]+1, inci[4], inci[5])
            print '\n'
            print '*' * 80
